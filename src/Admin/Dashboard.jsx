...
export default function Dashboard() {
  ...
  const statCards = [
    { label: "Total Doctors",      value: stats.totalDoctors || 0,             icon: <MdLocalHospital />, light: "bg-sky-50 text-sky-600" },
    { label: "Total Patients",     value: stats.totalPatients || 0,            icon: <FiUsers />,         light: "bg-violet-50 text-violet-600" },
    { label: "Total Appointments", value: stats.totalAppointments || 0,        icon: <FiCalendar />,      light: "bg-emerald-50 text-emerald-600" },
    { label: "This Month",         value: stats.thisMonthAppointments || 0,    icon: <FiTrendingUp />,    light: "bg-orange-50 text-orange-600" },
    { label: "Total Revenue",      value: `৳${(stats.totalRevenue || 0).toLocaleString()}`, icon: <FiDollarSign />, light: "bg-pink-50 text-pink-600" },
    { label: "Online Doctors",     value: stats.onlineDoctors || 0,            icon: <FiUserCheck />,     light: "bg-teal-50 text-teal-600" },
  ];

  if (loading) {
    return (
      <div className="p-8 px-60 py-60">
        <div className="text-slate-400 text-sm mb-4">Loading dashboard...</div>
  ...
  return (
    <div className="font-sans p-8 px-20 py-20">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-5">
        {statCards.map(({ label, value, icon, light }) => (
          ...
          {topDoctors.length === 0 ? (
            ...
            topDoctors.map((doc, i) => (
              <div key={doc._id}
                ...
                {/* Photo */}
                {doc.photo ? (
                  <img src={doc.photo} alt={doc.name} className="w-8 h-8 rounded-lg object-cover shrink-0" />
                ) : (
                  <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-sky-400 to-violet-500 flex items-center justify-center text-white text-xs font-bold shrink-0">
                    {doc.name?.[0]?.toUpperCase() || "D"}
                  </div>
                )}
                {/* Info */}
                <div className="flex-1 min-w-0">
                  <div className="text-sm font-semibold text-slate-800 truncate">Dr. {doc.name}</div>
                  <div className="text-xs text-slate-400">{doc.specialist} · {doc.count} appts</div>
                </div>
                {/* Mini bar */}
